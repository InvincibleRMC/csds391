    ...
    for j= 1:5
       ...
       % ave_chance =  zeros(100,1);
        ...
        for k = 1:100
            ...
            ave_hi = ave_hi +hi_history;
            %ave_hi = ave_hi / sum(ave_hi);
            %ave_chance(:,:,i) = chanceOfLime;
           % ave_chance = ave_chance/sum(ave_chance);
        end
        
       % new_ave = zeros(100,5);

        
        %new_ave = new_ave + sum(ave_hi,3);
        

       % ave_hi= new_ave;
        ave_hi = normr(ave_hi);
      % ave_chance= mean(ave_chance,3)
      % ave_chance = normc(ave_chance);
